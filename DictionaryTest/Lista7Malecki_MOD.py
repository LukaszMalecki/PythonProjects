...
while not inputGood:
    try:
        liczbaElementow = input("Wpisz ile elementow chcesz wpisac do krotki all_cases:\n")
        liczbaElementow = int(liczbaElementow)
        if liczbaElementow <= 0:
            raise ValueError("Niepoprawne: podano liczbe niedodatnia")
        else:
            inputGood = True
    except:
...
def dodajDoCases(n, sourceL, toModifyL):
    temp = list()
    for i in range(0, n):
        if i == 0:
            temp.append(sourceL[6])
        elif i >= 1 and i <= 3:
            temp.append(int(sourceL[4-i]))
        elif i == 4:
            temp.append( int(sourceL[5]) )
        elif i == 5:
            temp.append( int(sourceL[4]) )
        elif i == 6:
            temp.append( sourceL[0] )
        elif i < 11:
            temp.append(sourceL[i])
        elif i == 11:
            t = sourceL[i].replace("\n", "")
            if len(t) != 0:
                temp.append(float(t.replace(",",".")))
            #temp.append(sourceL[i].replace("\n", ""))
    toModifyL.append( tuple(temp))

def dodajDoCasesV2(n, sourceL, toModifyL):
    temp = list()
    for i in range(0, n):
        if i < 11:
            if sourceL[i].isnumeric():
                temp.append(int(sourceL[i]))
            else:
                temp.append(sourceL[i])
        elif i == 11:
            t = sourceL[i].replace("\n", "")
...
with open('.\Covid.txt', "r", encoding="utf-8") as f:
    ...
    for line in f:
        l = line.split("\t")
        if temp:
            dodajDoCasesV2(liczbaElementow, l, all_cases)
...
if len(l) != 12:
            ...
            dodajDoCases(liczbaElementow, l, all_cases)
            #all_cases.append( (l[6], int(l[3]), int(l[2]), int(l[1]), int(l[5]), int(l[4]) ) )
            by_date[(int(l[3]), int(l[2]), int(l[1]))].append((l[6], int(l[5]), int(l[4])))
            by_country[l[6]].append( (int(l[3]), int(l[2]), int(l[1]), int(l[5]), int(l[4])) )
'''
